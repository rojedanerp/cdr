...
// ============================================
// REPORTE GENERAL — resumen combinado de TODO el historial (remesas, caja
// y billetera), sin importar el periodo ni los filtros avanzados que estén
// activos en el resto de la sección Reportes. Se recalcula cada vez que
// cambian las remesas o los movimientos de caja (que es de donde sale tanto
// el saldo de caja como la billetera de USDT).
// ============================================
const repGenStatRemesas = document.getElementById('repGenStatRemesas');
const repGenStatClientes = document.getElementById('repGenStatClientes');
const repGenStatGanancia = document.getElementById('repGenStatGanancia');
const repGenStatCajaClp = document.getElementById('repGenStatCajaClp');
const repGenStatCajaUsdt = document.getElementById('repGenStatCajaUsdt');
const repGenStatUsdt = document.getElementById('repGenStatUsdt');
const repGenStatClpInvertido = document.getElementById('repGenStatClpInvertido');
const repGenStatTasaPromedio = document.getElementById('repGenStatTasaPromedio');
const repGenMonedaWrap = document.getElementById('repGenMonedaWrap');
const repGenMonedaBody = document.getElementById('repGenMonedaBody');
const repGenMonedaEmpty = document.getElementById('repGenMonedaEmpty');
const repGenCajaWrap = document.getElementById('repGenCajaWrap');
const repGenCajaBody = document.getElementById('repGenCajaBody');
const repGenCajaEmpty = document.getElementById('repGenCajaEmpty');

let reporteGeneralExportState = { stats: {}, resumenMoneda: [], caja: [] };

function renderizarReporteGeneral() {
    if (!repGenStatRemesas) return;

    // Se excluyen las remesas canceladas, igual que en el resto de Reportes.
    const remesasActivas = Object.values(remesasPorId).filter(r => r.estado !== 'cancelado');

    repGenStatRemesas.textContent = remesasActivas.length;

    const clientesUnicos = new Set(remesasActivas.map(r => r.clienteNombre).filter(Boolean)).size;
    repGenStatClientes.textContent = clientesUnicos;

    // --- Ganancia neta histórica, por moneda de envío ---
    const conReferencia = remesasActivas.filter(r =>
        r.tasaReferencia != null && r.tasaReferencia > 0 && r.tasaCambio != null && r.montoRecibido != null
    );
    const gananciaPorMoneda = {};
    conReferencia.forEach(r => {
        const moneda = (r.monedaEnviado || '').toUpperCase();
    ...
    const entradasGanancia = Object.entries(gananciaPorMoneda);
    repGenStatGanancia.textContent = entradasGanancia.length === 0
        ? '—'
        : entradasGanancia.map(([moneda, total]) => formatMoney(total, moneda)).join(' · ');

    // --- Resumen por moneda: todo lo que entró y salió, histórico completo ---
    const grupos = {};
    const getGrupo = (moneda) => {
        if (!grupos[moneda]) {
            grupos[moneda] = { moneda, entradasCount: 0, entradasTotal: 0, salidasCount: 0, salidasTotal: 0, comisionTotal: 0 };
        }
        return grupos[moneda];
    };
    remesasActivas.forEach(r => {
        const monedaEnv = (r.monedaEnviado || '').toUpperCase();
        const monedaRec = (r.monedaRecibido || '').toUpperCase();
        if (monedaEnv && r.montoEnviado != null) {
            const g = getGrupo(monedaEnv);
    ...
    const filasMoneda = Object.values(grupos).sort((a, b) => a.moneda.localeCompare(b.moneda));

    repGenMonedaBody.innerHTML = '';
    if (filasMoneda.length === 0) {
        repGenMonedaWrap.style.display = 'none';
        repGenMonedaEmpty.style.display = 'block';
    } else {
        repGenMonedaWrap.style.display = 'block';
        repGenMonedaEmpty.style.display = 'none';
        filasMoneda.forEach(f => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td>${escapeHtml(f.moneda)}</td>
                <td>${f.entradasCount}</td>
                ...
                <td class="mono-cell">${formatMoney(f.comisionTotal, f.moneda)}</td>
            `;
            repGenMonedaBody.appendChild(tr);
        });
    }

    // --- Saldo actual en caja (mismo cálculo que la sección Caja: respeta
    // la apertura vigente o el último cierre contado) ---
    const { saldosPorMoneda } = calcularSaldosActuales(movimientosCajaActuales);
    const monedasCaja = Object.keys(saldosPorMoneda).sort();

    repGenCajaBody.innerHTML = '';
    if (monedasCaja.length === 0) {
        repGenCajaWrap.style.display = 'none';
        repGenCajaEmpty.style.display = 'block';
    } else {
        repGenCajaWrap.style.display = 'block';
        repGenCajaEmpty.style.display = 'none';
        monedasCaja.forEach(m => {
            const tr = document.createElement('tr');
            tr.innerHTML = `<td>${escapeHtml(m)}</td><td class="mono-cell">${formatMoney(saldosPorMoneda[m], m)}</td>`;
            repGenCajaBody.appendChild(tr);
        });
    }

    // Saldo total de caja convertido a CLP y a USDT (usando las tasas
    // configuradas), igual que en el Dashboard.
    let totalClp = 0, totalClpCompleto = true;
    let totalUsdt = 0, totalUsdtCompleto = true;
    monedasCaja.forEach(m => {
        const enClp = convertirMoneda(saldosPorMoneda[m], m, 'CLP');
        if (enClp === null) totalClpCompleto = false; else totalClp += enClp;
        const enUsdt = convertirMoneda(saldosPorMoneda[m], m, 'USDT');
        if (enUsdt === null) totalUsdtCompleto = false; else totalUsdt += enUsdt;
    });
    repGenStatCajaClp.textContent = monedasCaja.length === 0 ? '—' : formatMoney(totalClp, 'CLP') + (totalClpCompleto ? '' : ' *');
    repGenStatCajaUsdt.textContent = monedasCaja.length === 0 ? '—' : formatMoney(totalUsdt, 'USDT') + (totalUsdtCompleto ? '' : ' *');

    // --- Billetera USDT: totales históricos (mismo cálculo que la sección
    // Billetera, a partir de los movimientos de caja marcados con origen
    // "compra_usdt" / "venta_usdt") ---
    let clpInvertidoTotal = 0, usdtCompradoTotal = 0;
    let entradasUsdt = 0, salidasUsdt = 0;
    ...
    const saldoUsdtBilletera = entradasUsdt - salidasUsdt;
    repGenStatUsdt.textContent = formatMoney(saldoUsdtBilletera, 'USDT');
    repGenStatClpInvertido.textContent = formatMoney(clpInvertidoTotal, 'CLP');
    repGenStatTasaPromedio.textContent = usdtCompradoTotal > 0
        ? formatMoney(clpInvertidoTotal / usdtCompradoTotal, 'CLP')
        : '—';

    // --- Snapshot para exportar (PDF/Excel) ---
    reporteGeneralExportState = {
        stats: {
            remesas: repGenStatRemesas.textContent,
            clientes: repGenStatClientes.textContent,
            ganancia: repGenStatGanancia.textContent,
            cajaClp: repGenStatCajaClp.textContent,
            cajaUsdt: repGenStatCajaUsdt.textContent,
            saldoUsdtBilletera: repGenStatUsdt.textContent,
            clpInvertido: repGenStatClpInvertido.textContent,
            tasaPromedio: repGenStatTasaPromedio.textContent
        },
        resumenMoneda: filasMoneda.map(f => ({
            moneda: f.moneda,
            entradasCount: f.entradasCount,
            entradasTotal: moneyTexto(f.entradasTotal, f.moneda),
            salidasCount: f.salidasCount,
        ...
        caja: monedasCaja.map(m => ({ moneda: m, saldo: moneyTexto(saldosPorMoneda[m], m) }))
    };
}

// --- Exportar Reporte general — PDF (jsPDF) y Excel (SheetJS) ---
const repGenExportarPdfBtn = document.getElementById('repGenExportarPdfBtn');
const repGenExportarExcelBtn = document.getElementById('repGenExportarExcelBtn');

repGenExportarPdfBtn.addEventListener('click', () => {
    const { stats, resumenMoneda, caja } = reporteGeneralExportState;
    if (Number(stats.remesas) === 0 && resumenMoneda.length === 0 && caja.length === 0) {
    ...
    doc.setFontSize(14);
    doc.text('Reporte general — todo el historial', 14, 15);
    doc.setFontSize(10);
    doc.setTextColor(120);
    doc.text(`Generado el ${new Date().toLocaleDateString('es-CL')}`, 14, 21);

    doc.autoTable({
        startY: 26,
        head: [['Remesas', 'Clientes', 'Ganancia neta', 'Caja ≈ CLP', 'Caja ≈ USDT', 'Saldo USDT billetera', 'CLP invertido', 'Tasa prom. compra']],
        body: [[stats.remesas, stats.clientes, stats.ganancia, stats.cajaClp, stats.cajaUsdt, stats.saldoUsdtBilletera, stats.clpInvertido, stats.tasaPromedio]],
    ...
    if (resumenMoneda.length > 0) {
        doc.setFontSize(11);
        doc.setTextColor(0);
        doc.text('Resumen por moneda (histórico)', 14, cursorY);
        doc.autoTable({
            startY: cursorY + 4,
    ...
    if (caja.length > 0) {
        ...
        doc.setFontSize(11);
        doc.setTextColor(0);
        doc.text('Saldo actual en caja', 14, cursorY);
        doc.autoTable({
            startY: cursorY + 4,
            head: [['Moneda', 'Saldo actual']],
            body: caja.map(c => [c.moneda, c.saldo]),
            styles: { fontSize: 8.5, cellPadding: 3 },
            headStyles: { fillColor: [30, 41, 59] },
            alternateRowStyles: { fillColor: [245, 246, 248] }
        });
...
repGenExportarExcelBtn.addEventListener('click', () => {
    const { stats, resumenMoneda, caja } = reporteGeneralExportState;
    if (Number(stats.remesas) === 0 && resumenMoneda.length === 0 && caja.length === 0) {
        alert('Todavía no hay datos para exportar.');
        return;
    ...
    const hojaResumen = XLSX.utils.json_to_sheet([{
        'Remesas totales': stats.remesas,
        'Clientes atendidos': stats.clientes,
        'Ganancia neta histórica': stats.ganancia,
        'Caja total (≈ CLP)': stats.cajaClp,
        'Caja total (≈ USDT)': stats.cajaUsdt,
        'Saldo USDT (billetera)': stats.saldoUsdtBilletera,
        'CLP invertido en USDT': stats.clpInvertido,
        'Tasa promedio de compra': stats.tasaPromedio
    }]);
    hojaResumen['!cols'] = [{ wch: 16 }, { wch: 18 }, { wch: 20 }, { wch: 16 }, { wch: 16 }, { wch: 20 }, { wch: 18 }, { wch: 18 }];
    XLSX.utils.book_append_sheet(libro, hojaResumen, 'Resumen general');

    if (resumenMoneda.length > 0) {
        const hojaMoneda = XLSX.utils.json_to_sheet(resumenMoneda.map(f => ({
            Moneda: f.moneda,
        ...
        hojaMoneda['!cols'] = [{ wch: 10 }, { wch: 12 }, { wch: 18 }, { wch: 12 }, { wch: 18 }, { wch: 16 }];
        XLSX.utils.book_append_sheet(libro, hojaMoneda, 'Resumen por moneda');
    }

    if (caja.length > 0) {
        const hojaCaja = XLSX.utils.json_to_sheet(caja.map(c => ({ Moneda: c.moneda, 'Saldo actual': c.saldo })));
        hojaCaja['!cols'] = [{ wch: 10 }, { wch: 18 }];
        XLSX.utils.book_append_sheet(libro, hojaCaja, 'Saldo en caja');
    }

    XLSX.writeFile(libro, `reporte-general-${fechaArchivo()}.xlsx`);
});
